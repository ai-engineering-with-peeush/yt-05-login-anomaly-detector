"""
generate_logs.py — synthetic auth-log generator.

Run via:  make data   (or: python src/generate_logs.py)

Outputs
-------
data/logs.csv    — all login events (~30 k rows)
data/labels.csv  — ground-truth attack labels (never read by the detector)

Users
-----
40 users across 10 cities, 30 days of normal activity.
Most work 9-18; a handful have irregular hours (on-call / shift workers) —
these are the users where the contextual anomaly detector *misses*, which
drives the episode 2 story.

Three injected attack patterns
-------------------------------
1. impossible_travel        — user logs in from home, then 30-50 min later
                               from a city 8 000+ km away
2. new_device_odd_hour      — unknown device at 1-4 AM; injected into both
                               regular-schedule users (should be caught) and
                               irregular-schedule users (often missed — that's
                               the point)
3. credential_stuffing      — 500 rapid login attempts across many accounts in
                               a 10-minute burst window; captured via burst_rate
                               aggregate feature
"""
...
DAYS = 30
EVENTS_PER_USER_PER_DAY_MU = 22   # Poisson mean
DATA_DIR = Path(__file__).parent.parent / "data"

# Cities used as home locations for each user group
...
# Far-away cities used only for impossible-travel injection
...
def _build_users() -> Dict[str, dict]:
    ...
    for city, lat, lon in CITIES:
        # 4 users per city: 3 regular schedule, 1 irregular
        for slot in range(4):
            user_id = f"user_{uid:03d}"
            irregular = (slot == 3)          # 1 in 4 is on-call / shift
...
def _random_ts_in_window(date: datetime, start_h: int, end_h: int) -> datetime:
    """Random timestamp within [start_h, end_h) on the given date."""
...
def _jitter_location(lat: float, lon: float, sigma_km: float = 5.0) -> Tuple[float, float]:
    """Small Gaussian noise around home location (simulates VPN / nearby office)."""
...
def inject_impossible_travel(events: List[dict], n: int = 12) -> List[dict]:
    """Add n impossible-travel pairs: home login, then 30-50 min later far away."""
    injected: List[dict] = []
    users = list(USERS.keys())
    random.shuffle(users)
    targets = users[:n]

    # Use events from days 20-29 so they land in the test window
    ...
    for user_id in targets:
        ...
        injected.append({
            ...
            "user_id":   user_id,
            "ts":        attack_ts,
            "device_id": anchor["device_id"],       # same device, different continent
            "country":   distant[0],
...
def inject_new_device_odd_hour(events: List[dict], n: int = 20) -> List[dict]:
    """Add n new-device-at-odd-hour events.

    Half target regular-schedule users (should be caught by the model),
    half target irregular-schedule users (model will likely miss — the contextual
    blind spot that episode 2 addresses).
    """
    injected: List[dict] = []
    ...
    for user_id in targets:
        ...
        injected.append({
            ...
            "user_id":   user_id,
            "ts":        ts,
            "device_id": f"unknown-{uuid.uuid4().hex[:8]}",  # brand-new device
            "country":   profile["country"],
            "lat":       round(lat, 5),
            "lon":       round(lon, 5),
            "success":   True,
...
def inject_credential_stuffing(n_attempts: int = 500) -> List[dict]:
    """Add a credential-stuffing burst: n_attempts spread across all users in 10 min.

    Each individual event looks unremarkable on its own — it's only the aggregate
    count of distinct accounts hit in a short window that reveals the attack.
    """
    injected: List[dict] = []
    burst_start = START_DATE + timedelta(days=25, hours=3, minutes=14)  # 3:14 AM
    users = list(USERS.keys())

    for i in range(n_attempts):
        user_id = random.choice(users)
        profile = USERS[user_id]
        offset_sec = random.uniform(0, 600)     # spread over 10 min
        ts = burst_start + timedelta(seconds=offset_sec)
        # Random IPs → random locations (attacker uses proxies)
        ...
        injected.append({
            ...
            "lat":       round(lat, 5),
            "lon":       round(lon, 5),
            "success":   random.random() < 0.02,  # 2% success — mostly failures
